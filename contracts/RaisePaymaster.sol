...
contract RaisePaymaster is IPaymaster {
    uint256 constant private PRICE_FOR_PAYING_FEES = 1;

    address public allowedToken;

    modifier onlyBootloader() {
        require(msg.sender == BOOTLOADER_FORMAL_ADDRESS, "Caller is not the bootloader");
        _;
    }
    ...
    function validateAndPayForPaymasterTransaction(bytes32 _txHash, bytes32 _suggestedSignedHash, Transaction calldata _transaction) external payable override onlyBootloader returns (bytes memory context) {
        ...
        bytes4 paymasterInputSelector = bytes4(bytes(_transaction.paymasterInput[0:4]));
        if (paymasterInputSelector == IPaymasterFlow.approvalBased.selector) {
            ...
            require(providedAllowance >= PRICE_FOR_PAYING_FEES, "Not enough allowance");
            ...
            require(success, "Funds transferring failed");
        } else {
